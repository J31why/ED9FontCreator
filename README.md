...
不过不能直接使用纹理文件, 需要dds转换后才能使用.  
云豹版Nisa版测试可用, 副作用不详. 纹理尺寸用的是Nisa版尺寸: 4096*4096  
  
![main](Screenshots/main.png) 

# 前置
 .net8和[TexconvGui](https://github.com/bj-rn/texconvgui)
# 使用
### FNT解析
...
默认设置是我个人使用的思源宋体.  
字体边距越大字符周围空白越大, 可以输入负数缩减字符边距(缩减太多, 可能会引发游戏裁剪错误, 导致字符有杂点).
...
### 生成
以上设定完成后再进行下方操作:  
1. 生成红色字符, 等黑色页面出现红色字符后查看是否超过底部的边界, 如果超过了就需要调整字体大小或边距, 之后导出字符图片.   
2. 生成绿色字符, 与上方同样的操作.  
3. 合成字符图片
